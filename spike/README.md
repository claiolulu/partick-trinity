...
每站可以配一组到现场做的任务，访客逐条勾掉。

```jsonc
"tasks": [
  { "kind": "look",  "text": "抬头数一数穹顶从中心辐射出多少根铸铁肋" },
  { "kind": "find",  "text": "在门楣或奠基石上找到落成年份" },
  { "kind": "photo", "text": "走到街对面，用竖构图把整座塔收进一张照片" }
]
```

`kind` 只影响那个小标签：`look` 观察 / `find` 找到 / `photo` 拍照 / `walk` 走一段 / `feel` 体验。

页面上：内容卡里是可勾的清单，收起的窄条上显示 `2/4`，全做完变橙；
chip 上做完的站带 ✓；卡片底部有全程进度和"清空本路线打卡"。

> ⚠️ **打卡状态只存在访客自己那台设备的 `localStorage` 里。**
> 这是纯静态站点，没有后端，所以：
> - 刷新、关掉再回来都还在
> - **换手机 / 换浏览器 / 无痕模式 = 从零开始**
> - **人和人之间互相看不到进度，没有排行榜、没有统计**
>
> 要做多人共享、要知道有多少人走完，必须上后端（或者接一个 BaaS）。
> 那是另一件事，不是改几行前端能解决的。

写任务时的一个经验：**尽量写"自验证"的任务** —— "数一数有几根"、"找到年份记下来"
这种，做没做只有本人知道，也不需要我们预先知道正确答案。
避免写"找到写着 XX 的牌子"，因为我们并不确定那块牌子存在。
