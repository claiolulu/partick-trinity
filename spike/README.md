...
**不画房间**，只把楼按层切开、逐层错开一点，像抽屉拉出来一半。
视觉上有"能看进去"的意思，但它明摆着是示意 —— 所以不涉及版权，也不用假装精度。

```jsonc
"cutaway": {
  "buildings": { "osmIds": ["way/43059928"] },  // 只剖其中一栋（不写=这站全部）
  "offset": 5.5,        // 每层往外错多少米
  "bearing": 55,        // 错开方向
  "gap": 0.3,           // 层间留多少比例的空气
  "zScale": 2.4,        // 竖向夸张倍数
  "levels": [
    { "name": "基座", "h": 1.2, "note": "这一层里发生什么" }
  ]
}
```

页面上会多一个「剖面」按钮，只在这一站真有数据时出现。打开时：
被剖的那几栋换成一摞板，**同一站其他没剖的楼留着当环境**，
相机切到按整摞包围盒算出来的机位，内容卡里列出每层的说明。

两个必须知道的点：

- **`zScale` 是必要的。** 真实层高只有 1~4 m，而横向要错开好几米，1:1 画出来会摊成一片而不是一摞。
- **页面会自己声明"分层是示意、竖向已夸张 N×、非实测"。** 别把这行删掉 —— 删了就成了假装精确。
